...
kf=0.01; % frequency sensitivity of the NBFM modulation
theta=2*pi*y_int*kf; % phase angle of the carrier signal
msg_fm=cos(2*pi*fc*t+theta.'); % NBFM modulation

%-----------------PLOT THE NBFM MODULATED AUDIO FILE---------------------%
figure
subplot(2,1,1)
plot(t,msg_fm) % plot the NBFM modulated audio file in time domain
title('NBFM modulated audio file in time domain')
xlabel('Time (s)')
ylabel('Amplitude')
subplot(2,1,2)
plot(x,abs(fftshift(fft(msg_fm)))) % plot the NBFM modulated audio file in frequency domain
title('NBFM modulated audio file in frequency domain')
xlabel('Frequency (Hz)')
ylabel('Amplitude')
%-----------------NBFM DEMODULATION---------------------------------------%
% differentiating the NBFM modulated signal
msg_diff=diff(msg_fm); %differentiating NBFM to AM signal
envelope=abs(hilbert(msg_diff)); % envelope detection
msg_demod=envelope-mean(envelope); % DC blocking
msg_demod=msg_demod/(2*pi*kf);
%-----------------PLAY THE NBFM DEMODULATED AUDIO FILE--------------------%
msg_resample=resample(msg_demod,Fs,new_Fs); % resample the audio file
disp('Playing the NBFM demodulated audio file')
sound(msg_resample,Fs)
pause(length(msg_resample)/Fs)
%-----------------PLOT THE NBFM DEMODULATED AUDIO FILE---------------------%
t=0:1/Fs:length(msg_resample)/Fs-1/Fs; % time axis
x=-Fs/2:Fs/length(msg_resample):Fs/2-Fs/length(msg_resample);%frequency axis
figure
subplot(2,1,1)
plot(t,msg_resample) % plot the NBFM demodulated audio file in time domain
title('NBFM demodulated audio file in time domain')
xlabel('Time (s)')
ylabel('Amplitude')
subplot(2,1,2)
plot(x,abs(fftshift(fft(msg_resample)))) % plot the NBFM demodulated audio file in frequency domain
title('NBFM demodulated audio file in frequency domain')
xlabel('Frequency (Hz)')
ylabel('Amplitude')
